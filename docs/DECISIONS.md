...
|---|---|---|---|
| D1 | Rule interpretations I1–I7 | Rules | active |
| D8 | Three end states | Rules | active |
| D43 | Random players never make illegal moves | Rules | active |
| D2 | Pure functions over an immutable state | Engine | active |
| D3 | Disks are plain integers; no ownership after setup | Engine | active |
| D4 | Fixed action space of seven, player-relative | Engine | active |
| D22 | `State` hashes by content despite dict fields | Engine | active |
| D23 | `_validate` raises, `_illegal_reason` explains | Engine | active |
| D24 | `bool` is not an int | Engine | active |
| D25 | Engine size | Engine | active |
| D5 | Agent contract | Agents and runner | active |
| D6 | One runner | Agents and runner | active |
| D9 | External agent with timeout, fallback, move source | Agents and runner | active |
| D26 | Agents carry `kind` and `last_fell_back` | Agents and runner | active |
| D27 | The injected `ask` owns the waiting | Agents and runner | active |
| D28 | Stalemate counts (state, player) pairs | Agents and runner | active |
| D29 | Status checked before each entry and after the loop | Agents and runner | active |
| D32 | `run` gains `on_turn` | Agents and runner | active |
| D44 | Ending a game early (`StopGame`) | Agents and runner | active |
| D7 | Recording format | Recording | active |
| D30 | Validation in `Recording.__post_init__` | Recording | active |
| D31 | `replay` is `run` with scripted agents | Recording | active |
| D18 | Autosave | Recording | active |
| D35 | Continuation recordings are the whole game | Recording | active |
| D36 | `hanoi recordings` replays each file | Recording | active |
| D12 | CLI shape | CLI | active |
| D13 | Schedule flags | CLI | active |
| D14 | Seed default 0 | CLI | active |
| D15 | Repetition limit default 10 | CLI | superseded by D42 |
| D16 | Move timeout default 30 s | CLI | active |
| D33 | Stdin timeout via reader thread | CLI | active |
| D34 | End of input counts as no answer | CLI | active |
| D41 | `--max-turns` defaults to 200 · 3ⁿ | CLI | active |
| D42 | Repetition limit off by default | CLI | active |
| D17 | Drawn towers by default | Output | active |
| D38 | Full game trace by default | Output | superseded by D40 |
| D39 | Bot turns spell out their effect | Output | active |
| D40 | Default output is the final state only | Output | active |
| D10 | Tooling | Process | active |
| D11 | TDD visible in git | Process | active |
| D21 | Where the documents live | Documentation | active |
| D37 | README structure | Documentation | active |
| D19 | Interfaces and their purposes | Scope | active |
| D20 | Scope beyond the spec | Scope | active |
